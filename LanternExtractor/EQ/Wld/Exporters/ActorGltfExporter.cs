...
namespace LanternExtractor.EQ.Wld.Exporters
{
    public static class ActorGltfExporter
    {
        ...
        private static void ExportZone(WldFileZone wldFileZone, Settings settings, ILogger logger)
        {
            ...
            var bspNodes = wldFileZone.GetFragmentsOfType<BspTree>()[0].Nodes;
            gltfWriter.ConstructBoundingBoxTree(zoneMeshes, bspNodes);
            var zoneNodes = bspNodes.Where(n => n.Region?.RegionType?.Zoneline?.Type == DataTypes.ZonelineType.Reference && n.Region?.RegionType?.Zoneline?.Index > 0);

            if (settings.ExportZoneMeshGroups)
            {
                //foreach (var node in bspNodes)
                //{
                //    if (node.Region?.RegionType?.Zoneline != null && node.Region.Mesh == null)
                //    {
                //        var nextNode = node.IsLeftChild ? node.Parent.RightChild : node.Parent.LeftChild;
                //        if (nextNode.Region.Mesh != null)
                //        {
                //            gltfWriter.AddRegionData(nextNode.Region.Mesh, node);
                //        }
                //    }
                //    else if (node.Region?.Mesh != null)
                //    {
                //        gltfWriter.AddRegionData(node.Region.Mesh, node);
                //    }
                //}

                var rootNode = bspNodes.FirstOrDefault();
                if (rootNode != null)
                {
                    File.WriteAllText($"{wldFileZone.GetExportFolderForWldType()}aabb_tree.json", bspNodes.FirstOrDefault()?.Serialize(true));
                }
            }
            var animatedNames = new HashSet<string>();
            Action<string> addToAnimatedNames = s => {
                s = s.ToLower().Split('_').FirstOrDefault();
                animatedNames.Add(s); 
            ...
            File.WriteAllText($"{wldFileZone.GetExportFolderForWldType()}animated_objects.json", JsonSerializer.Serialize(animatedNames));
        ...
        private static void ExportSkeletalActor(Actor actor, Settings settings, WldFile wldFile, ILogger logger)
        {
            ...
            if (wldFile.ZoneShortname == "global") { 
                  // GlobalReference.CharacterWld.GetInjectedWldsAssociatedS3dArchives
            }
            ...
            gltfWriter.GenerateGltfMaterials(materialLists, textureImageFolder, true);
            ...
            if (skeleton.Meshes != null)
            {
                foreach (var mesh in skeleton.Meshes)
                {
                    ...
                    for (var i = 0; i < mesh.MaterialList.VariantCount; i++)
                    {
                        gltfWriter.AddFragmentData(mesh, skeleton, materialVariation: i, secondaryMesh: false);
                    }
                ...
                for (var i = 0; i < skeleton.SecondaryMeshes.Count; i++)
                {
                    ...
                    var mesh = skeleton.SecondaryMeshes[i];
                    var originalVertices = MeshExportHelper.ShiftMeshVertices(mesh, skeleton,
                       wldFile.WldType == WldType.Characters, "pos", 0);
                    gltfWriter.AddFragmentData(mesh, skeleton, materialVariation: i, secondaryMesh: true);
                    mesh.Vertices = originalVertices;

                }
            }

            if (true || settings.ExportAllAnimationFrames)
            {
